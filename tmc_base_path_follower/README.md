...
移動経路を"path_follow_action"アクションで受け取る。"~/path"トピックで受け取ることもできる。  
自己位置情報である"global_pose"トピックを購読し、ロボット移動速度指令値"base_velocity"トピックを生成し出版する。  
...
全方位モデルでは、台車中心のxy成分の並進速度指令と、yaw成分の旋回速度指令を独立に指令することができ、  
...
台車中心のx成分の並進速度指令と、yaw成分の旋回速度指令を独立に指令することができる。y成分は常に0を出力する。  
x成分により進行速度を制御する。  
...
#### 出版するトピック (トピック名 [型] : 説明)
- "base_velocity" [geometry_msgs/msg/Twist] : 移動速度指令値(並進速度[m/s]、旋回速度[rad/s])

#### 購読するトピック (トピック名 [型] : 説明)
- "global_pose" [geometry_msgs/msg/PoseStamped] : 自己位置

- "~/path" [nav_msgs/msg/Path] : 追従経路

#### 提供するアクション(アクション名 [型] : 説明)
- "path_follow_action" [tmc_navigation_msgs/action/PathFollowerAction] : 経路追従アクション
   - goal:
      - nav_msgs/msg/Path path : 追従経路
   - result:
      - 無し
   - feedback:
...
- global_pose_timeout [double] [s] : 自己位置トピックが途絶したと判断する時間。途絶したらアクションはABORTEDを返し、経路追従中であった場合は停止する (default : 1.0)  
...
omni_velocity_calculator : 全方位モデル速度計算クラスパラメータ群  
      ...
      max_linear_velocity [double] [m/s]               : 最大並進速度 (default : 0.6)  
      max_angular_velocity [double] [rad/s]            : 最大旋回速度 (default : 0.8)  
      max_linear_acceleration [double] [m/s^2]         : 最大並進加速度 (default : 1.0)  
      max_angular_acceleration [double] [rad/s^2]      : 最大旋回加速度 (default : 1.0)  
      ...
      linear_velocity_margin [double] [m/s]            : 並進最低速度マージン (default : 0.05)  
      path_length_threshold [double] [m]               : 上体の向け方を変えるゴールまでの経路長閾値 (default : 2.0)  
      linear_p_gain [double]                           : 並進速度P制御ゲインパラメータ (default : 0.5)  
      angular_p_gain [double]                          : 旋回速度P制御ゲインパラメータ (default : 0.5)  
...
diff_drive_velocity_calculator : 差動二輪モデル速度計算クラスパラメータ群  
      ...
      angle_error_angular_velocity_rate [double]       : 角度誤差に対する旋回速度の比率 (default : 1.0)  
      spin_start_error_angle [double] [rad]            : その場旋回を開始する角度誤差 (default : 0.8)  
      spin_end_error_angle [double] [rad]              : その場旋回を終了する角度誤差 (default : 0.03)  
      spin_max_angular_velocity [double] [rad/s]       : その場旋回の最大旋回速度 (default : 2.0)  
      spin_min_angular_velocity [double] [rad/s]       : その場旋回の最低旋回速度 (default : 0.05)  
...
omni_goal_checker : 全方位モデルゴール判定クラスパラメータ群  
      ...
      goal_area_length [double] [m]                    : ゴールエリア判定距離 (default : 0.5)  
      goal_line_length [double] [m]                    : ゴールエリア判定用ラインのゴールからの距離 (default : 0.05)  
      goal_stop_error_length [double] [m]              : ゴール判定用の位置偏差閾値 (default : 0.03)  
      goal_stop_error_angle [double] [rad]             : ゴール判定用の角度偏差閾値 (default : 0.03)  
...
nearest_path_point_searcher : 最寄り経路点探索クラスパラメータ群
      ...
      partial_search_range [double] [m]                : 前回の最寄り点からの探索範囲。0.0が指定された場合は常に全探索 (default : 0.0)  
      partial_search_permit_error [double] [m]         : 探索結果と自己位置との許容するエラー値。 許容値を超えた場合は全探索 (default : 0.5)  

path_transit_velocity_calculator : 経路通過速度算出クラスパラメータ群  

      max_linear_velocity [double] [m/s]               : 最大並進速度 (default : 0.6)  
      min_linear_velocity [double] [m/s]               : 最低並進速度 (default : 0.3)  
      max_angular_velocity [double] [rad/s]            : 最大旋回速度 (default : 0.8)  
      max_linear_acceleration [double] [m/s^2]         : 最大並進加速度 (default : 1.0)  
      max_angular_acceleration [double] [rad/s^2]      : 最大旋回加速度 (default : 1.0)  
